...
<main>
    <ul class="mt-10 grid grid-cols-3 items-center border-b-2 border-b mx-20 mb-6">
        <li class="justify-self-end text-xl font-bold mb-4"><a href="index.php?action=catalogue"><button class="text-white bg-black  px-5 py-3 rounded-[25px] font-bold ">Artistes</button></a></li>
        ...
        <li class="justify-self-start text-xl font-bold mb-4"><a href="index.php?action=playlist"><button class="text-black bg-white px-5 py-3 rounded-[25px] font-bold hover:font-black">Playlists</button></a></li>
    </ul>
    <div class="ml-20">
        ...
        <input id="cp" class="relative border border-[#FF742F] border-2 rounded-[5px] mt-3 py-2 px-12" type="text" placeholder="Recherchez un artiste">
    </div>
    ...
    <section class="grid grid-cols-5 grid-rows-2 mt-8 gap-y-6 items-center mx-14">
        ...
        foreach ($model as $value) {
            echo "<div class=\" artiste grid grid-cols-1 gap-y-6 justify-items-center hover:scale-105\"><img class=\"h-44 w-56 opacity-90 rounded-[25px] drop-shadow-xl items-center\" src=\"pictures/Shiny_overlay.svg\" alt=\"cover musique\"></br><a href='index.php?action=infos-artiste&name=" . $value->ArtistId . "' class='absolute items-center mt-20 text-lg text-white font-black'>";
            echo $value->artiste;
            echo "</a></div>";
        }
        ?>
    ...
    <script>
        ...
        const genres = document.querySelectorAll('.artiste');
